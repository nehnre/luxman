...
$(function(){
    var bpId = nehnre.queryString()["bpId"];
    loadData(bpId);
    $("#btnCancle").click(function(){
        location.href="BrandProject.aspx?_" + new Date().getTime();
    });
    $("#btnSubmit").click(function(){
        ...
        data += "&bpType=" + $("#bpType").val();
        data += "&bpContent=" + nehnre.encode(KE.html("bpContent"));
        $.ajax({
...
function loadData(bpId){
    if(!bpId) return;
    var data = "method=detail&ajax=true&bpId=" + bpId;
    $.ajax({
        url:"../ashx/brandproject.ashx",
        cache:false,
        type:"POST",
...
function fillData(json){
    ...
    $("#bpType").val(data.bs_type);
    (function(){
        KE.html("bpContent", data.bp_content);
        if(!KE.html("bpContent") && data.bp_content){
